/// # Commerce Kit UI Examples
///
/// This file contains complete UI examples that you can see and interact with.
/// These demonstrate the visual components of the Commerce Kit package.
library;
...
class CommerceKitUIExamples extends StatelessWidget {
  const CommerceKitUIExamples({super.key});

  @override
  Widget build(BuildContext context) {
    return ProviderScope(
      child: MaterialApp(
        title: 'Commerce Kit UI Examples',
        theme: ThemeData(
...
class ExamplesHomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Commerce Kit UI Examples'),
          ...
          if (cartItems.isNotEmpty)
            CartSummaryWidget(
              breakdown: PriceBreakdown(
                subtotal: cartTotal,
                total: cartTotal + const Money(5.99),
              ),
            ),
        ],
      ),
...
class _ProductDetailExamplePageState
  ...
  Widget build(BuildContext context) {
    ...
    final quantityInCart = ref.watch(
      cartItemCountProvider.select((count) => count),
    );
...
void initializeSampleData(WidgetRef ref) {
  ...
  notifier.addProduct(burger, quantity: 2);
  notifier.addProduct(fries);
  notifier.addProduct(drink, quantity: 2);
}
