...
/// How the sync bookkeeping database is opened. Override this to encrypt the
/// outbox at rest (it holds pending records — often the same sensitive data as
/// the domain DB), e.g.:
///
/// ```dart
/// syncDatabaseOptionsProvider.overrideWithValue(
///   const LocalDbOptions(
///     name: 'sync.sqlite',
///     encryption: EncryptionConfig.fromSecureStorage(storageKey: 'sync.cipher_key'),
///   ),
/// );
/// ```
final syncDatabaseOptionsProvider = Provider<LocalDbOptions>(
  name: 'syncDatabaseOptionsProvider',
  (ref) => const LocalDbOptions(name: 'sync.sqlite'),
);
...
final syncDatabaseProvider = Provider<SyncDatabase>(
  ...
  (ref) {
    final db = SyncDatabase(
      LocalDbKit.openExecutor(ref.watch(syncDatabaseOptionsProvider)),
    );
