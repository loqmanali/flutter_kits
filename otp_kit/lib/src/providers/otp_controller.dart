...
class OTPController extends StateNotifier<OTPState> {
  ...
  String? validate({List<OTPValidationRule>? customRules}) {
    return OTPValidator.validate(state.value, config, customRules: customRules);
  }

  /// Set focus state
  void setFocused(bool isFocused, [int focusedIndex = -1]) {
    state = state.copyWith(isFocused: isFocused, focusedIndex: focusedIndex);
  }
...
/// ```
final otpValueProvider = Provider.autoDispose.family<String, OTPConfig>((
  ref,
  config,
) {
  return ref.watch(
    otpControllerProvider(config).select((state) => state.value),
  );
});
...
/// ```
final otpIsCompleteProvider = Provider.autoDispose.family<bool, OTPConfig>((
  ref,
  config,
) {
  return ref.watch(
...
/// ```
final otpHasErrorProvider = Provider.autoDispose.family<bool, OTPConfig>((
  ref,
  config,
) {
  return ref.watch(
    otpControllerProvider(config).select((state) => state.hasError),
  );
});
...
/// ```
final otpErrorMessageProvider = Provider.autoDispose.family<String?, OTPConfig>(
  (ref, config) {
    return ref.watch(
...
/// ```
final otpProgressProvider = Provider.autoDispose.family<double, OTPConfig>((
  ref,
  config,
) {
  return ref.watch(
    otpControllerProvider(config).select((state) => state.progress),
  );
});
