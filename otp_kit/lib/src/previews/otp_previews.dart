...
@Preview(
  name: 'Default light · 4 cells',
  group: 'OTPTextField',
  size: previewSize,
)
Widget otpDefaultLight() => _scope(
  Builder(
    builder: (context) => OTPTextField(config: OTPTheme.defaultLight(context)),
  ),
);

@Preview(name: 'Modern · 6 cells', group: 'OTPTextField', size: previewSize)
Widget otpModern() => _scope(
  Builder(builder: (context) => OTPTextField(config: OTPTheme.modern(context))),
);

@Preview(name: 'Secure (obscured)', group: 'OTPTextField', size: previewSize)
Widget otpSecure() => _scope(
  Builder(builder: (context) => OTPTextField(config: OTPTheme.secure(context))),
);

@Preview(name: 'Premium', group: 'OTPTextField', size: previewSize)
Widget otpPremium() => _scope(
  Builder(
    builder: (context) => OTPTextField(config: OTPTheme.premium(context)),
  ),
);
...
Widget otpRtl() => _scope(
  Builder(
    builder: (context) {
      ...
      return OTPTextField(config: base.copyWith(isRTL: true));
    },
  ),
);
...
Widget resendIdle() =>
    _resendWith(const IdleResendState(attemptsUsed: 0, maxAttempts: 3));
