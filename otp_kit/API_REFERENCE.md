...
Behavior:
- Backed by a **single hidden text field**; the cells are a painted view of
  its value, sized adaptively to the available width. This makes typing
  strictly sequential, backspace reliable on mobile soft keyboards, and
  paste/SMS auto-fill a plain string insert — no focus choreography.
- The active cell is always `value.length`; deleting walks it back.
- The field carries `AutofillHints.oneTimeCode` (SMS auto-fill).
- Multi-character inserts (paste, auto-fill) are truncated to
  `config.length` and fill the cells directly.
- `customValidationRules` run when the code reaches full length; a failing
  rule sets the error state (shake + error border) instead of firing
  `onCompleted`.
- `enableAnimations: true` adds a scale pulse on the entered cell and a
  horizontal shake on `setError`.
- `showCursor: true` draws a blinking caret in the active cell.
...
- `autoSubmit` is a no-op kept for API compatibility — completion already
  fires `onCompleted`.
