...
/// {@template interceptor_logger_config}
/// Shared configuration and utilities for interceptor logging.
///
/// Provides ANSI color codes and standardized logging methods so multiple
/// interceptors can produce consistently formatted, easy-to-scan output.
/// {@endtemplate}
class InterceptorLoggerConfig {
  ...
  /// Standard box width for consistent formatting.
  static const int boxWidth = 73;

  /// Alternative box width for compatibility with PrettyDioLogger.
  ...
  /// Create PrettyDioLogger-style box (`╔╣ ║ ╚`).
...
${colorize('╚', color)}$line${colorize('╝', color)}''';
  ...
  static void logSection({
    ...
    final line = '─' * boxWidth;
    log(
      '$color┌── $title $line${_colors['reset']}',
    );
    for (final detail in details) {
      log('$color│ $detail${_colors['reset']}');
    }
    log(
      '$color└$line${_colors['reset']}',
    );
  }
