...
void main() {
  ...
  group('SlotDateConfig.isDayDisabled', () {
    ...
    test('predicate and disabledDates combine (either disables)', () {
      ...
      expect(
          config.isDayDisabled(DateTime(2026, 3, 15)), isTrue); // via predicate
  ...
  group('SlotItem', () {
    test('equality is by id only', () {
      ...
      final b =
          SlotItem(id: 1, date: DateTime(2030), time: '23:00', isBooked: true);
      final c = SlotItem(id: 2, date: DateTime(2026), time: '09:00');

      expect(a == b, isTrue,
          reason: 'same id → equal regardless of other fields');
      expect(a == c, isFalse);
    });
