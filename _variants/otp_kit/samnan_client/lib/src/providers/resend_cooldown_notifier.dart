...
class ResendCooldownNotifier extends StateNotifier<ResendState> {
  ResendCooldownNotifier({
    ...
    required ResendCooldownService service,
        ...
        super(IdleResendState(attemptsUsed: 0, maxAttempts: config.maxAttempts)) {
    _bootstrap();
  }
  ...
  Future<void> _bootstrap() async {
    ...
    _phaseEndsAt =
        DateTime.now().add(Duration(seconds: config.initialCountdownSeconds));
  ...
  Future<bool> recordResend() async {
    ...
    if (newAttempts >= config.maxAttempts) {
      final endsAt =
          DateTime.now().add(Duration(seconds: config.longCooldownSeconds));
      await _service.writeLongCooldownEnd(config.namespace, endsAt);
      _phaseEndsAt = endsAt;
      _emit(_buildLong(newAttempts));
    } else {
      _phaseEndsAt =
          DateTime.now().add(Duration(seconds: config.shortCooldownSeconds));
...
final resendCooldownProvider = StateNotifierProvider.family<
    ResendCooldownNotifier, ResendState, ResendCooldownConfig>((ref, config) {
