...
void main() {
  group('ShimmerShape', () {
    testWidgets('rectangle renders a Container with the given size', (
      tester,
    ) async {
    ...
    testWidgets('circle is laid out as 2*radius square', (tester) async {
      await tester.pumpWidget(_wrap(const ShimmerShape.circle(radius: 20)));
      final size = tester.getSize(find.byType(Container));
      expect(size.width, 40);
      expect(size.height, 40);
    });

    testWidgets('circle uses BoxShape.circle (no borderRadius)', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap(const ShimmerShape.circle(radius: 15)));
    ...
    testWidgets('rectangle uses a rounded rectangle decoration', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(
          const ShimmerShape.rectangle(width: 50, height: 50, borderRadius: 10),
        ),
      );
      final container = tester.widget<Container>(find.byType(Container));
      final decoration = container.decoration! as BoxDecoration;
    ...
    testWidgets('text shape defaults to height 16', (tester) async {
      await tester.pumpWidget(_wrap(const ShimmerShape.text(width: 80)));
      final size = tester.getSize(find.byType(Container));
  ...
  group('AppSpacing', () {
    ...
    testWidgets('flex() produces an Expanded with the given flex', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(
          const Column(children: [Text('a'), AppSpacing.flex(3), Text('b')]),
        ),
      );
  ...
  group('AppSpacingContextExtension', () {
    testWidgets('windowHeight/windowWidth reflect MediaQuery size', (
      tester,
    ) async {
      late double h;
      late double w;
