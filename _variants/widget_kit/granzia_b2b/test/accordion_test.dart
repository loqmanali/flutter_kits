...
void main() {
  group('Accordion', () {
    ...
    testWidgets('tapping a header expands that panel', (tester) async {
      ...
      expect(_panelHeightOf(tester, '0'), greaterThanOrEqualTo(_contentHeight), reason: 'panel 0 expanded');
    });

    testWidgets('tapping an expanded header collapses it again', (tester) async {
      await tester.pumpWidget(_accordion(allowMultipleOpen: false));
      await tester.pumpAndSettle();
    ...
    testWidgets('single-open mode: opening one closes the other',
      ...
      expect(_panelHeightOf(tester, '1'), greaterThanOrEqualTo(_contentHeight), reason: 'panel 1 expanded');
      expect(_panelHeightOf(tester, '0'), 0);
    });
