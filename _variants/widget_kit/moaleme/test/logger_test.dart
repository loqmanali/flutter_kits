...
void main() {
  ...
  group('custom log handler routing', () {
    test('receives messages at or above the threshold', () {
      ...
      AppLogger.setLogHandler((level, message, error, stack) => received.add(level));
      ...
      expect(received, [AppLogLevel.info, AppLogLevel.warning, AppLogLevel.error]);
    });
    ...
    test('passes the message text through to the handler', () {
      ...
      AppLogger.setLogHandler((level, message, error, stack) => captured = message);
