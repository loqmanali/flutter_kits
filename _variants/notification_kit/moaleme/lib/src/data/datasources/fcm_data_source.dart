...
class FCMDataSourceImpl implements FCMDataSource {
  ...
  Future<String?> getToken() async {
    // On iOS, FCM can only issue a token AFTER Apple delivers the APNS token.
    // Calling getToken() too early throws
    // `[firebase_messaging/apns-token-not-set]`. Wait (briefly, with retries)
    // for the APNS token first; if it never arrives (e.g. a Simulator, or
    // permission denied) return null instead of throwing — push is best-effort.
  ...
  /// Polls [FirebaseMessaging.getAPNSToken] until it is non-null. Returns false
  /// if it is still unavailable after [maxAttempts]. Mirrors the wait used in
  /// `NotificationInitializer` so token acquisition and topic subscription share
  /// the same iOS readiness gate.
