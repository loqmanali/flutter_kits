...
class NotificationInitializer {
  ...
  static Future<bool> initialize({
    ...
    if (_isInitialized) {
      NotificationKitRuntime.logger.debug('[${NotificationConfig.moduleName}] Already initialized');
      return true;
    }
    ...
    try {
      ...
      NotificationKitRuntime.logger.error(
        '[${NotificationConfig.moduleName}] Initialization failed: $e',
      );
      return false;
    }
  }
  ...
  static Future<void> setupRemoteNotifications({
    ...
    try {
      ...
      NotificationKitRuntime.logger.debug(
        '[${NotificationConfig.moduleName}] Remote notifications setup complete',
      );
    } catch (e) {
      NotificationKitRuntime.logger.error(
        '[${NotificationConfig.moduleName}] Remote setup failed: $e',
      );
    }
  }
  ...
  static Future<bool> _waitForAPNSToken({
    ...
    for (int attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (apnsToken != null) {
        NotificationKitRuntime.logger.debug(
          '[${NotificationConfig.moduleName}] APNS token available (attempt $attempt)',
        );
        return true;
      }
      NotificationKitRuntime.logger.debug(
        '[${NotificationConfig.moduleName}] Waiting for APNS token (attempt $attempt/$maxAttempts)',
      );
      await Future.delayed(delay);
    }
    NotificationKitRuntime.logger.error(
      '[${NotificationConfig.moduleName}] APNS token not available after $maxAttempts attempts',
    );
    return false;
  }
  ...
  static Future<NotificationSettings> _requestPermission() async {
    ...
    NotificationKitRuntime.logger.debug(
      '[${NotificationConfig.moduleName}] Permission status: ${settings.authorizationStatus}',
    );
  ...
  static Future<String?> _downloadAndSaveFile(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        NotificationKitRuntime.logger.error(
          '[NotificationInitializer] Failed to download image. Status: ${response.statusCode}',
        );
        return null;
      }
    } catch (e) {
      NotificationKitRuntime.logger.error('[NotificationInitializer] Error downloading image: $e');
      return null;
    }
  }
  ...
  static Future<void> _initializeLocalNotifications(
    ...
    NotificationKitRuntime.logger.debug(
      '[${NotificationConfig.moduleName}] Local notifications initialized',
    );
  }
  ...
  static Future<void> _createNotificationChannels() async {
    ...
    if (androidPlugin != null) {
      ...
      NotificationKitRuntime.logger.debug(
        '[${NotificationConfig.moduleName}] Notification channels created',
      );
    }
  }
  ...
  static Future<void> _subscribeToTopic(String topic) async {
    try {
      ...
      NotificationKitRuntime.logger.debug(
        '[${NotificationConfig.moduleName}] Subscribed to topic: $topic',
      );
    } catch (e) {
      NotificationKitRuntime.logger.error(
        '[${NotificationConfig.moduleName}] Failed to subscribe to $topic: $e',
      );
    }
  }
  ...
  static Future<void> _setForegroundOptions() async {
    ...
    NotificationKitRuntime.logger.debug(
      '[${NotificationConfig.moduleName}] Foreground options set',
    );
  }
  ...
  static void _defaultNotificationTapHandler(NotificationResponse response) {
    NotificationKitRuntime.logger.debug(
      '[${NotificationConfig.moduleName}] Notification tapped: ${response.payload}',
    );
  }
  ...
  static Future<String?> getFCMToken() async {
    try {
      ...
      if (Platform.isIOS) {
        ...
        if (!hasAPNS) {
          NotificationKitRuntime.logger.error(
            '[${NotificationConfig.moduleName}] Cannot get FCM token: APNS token not available',
          );
          return null;
        }
      }
      ...
      NotificationKitRuntime.logger.debug(
        '[${NotificationConfig.moduleName}] FCM token retrieved successfully',
      );
      return token;
    } catch (e) {
      NotificationKitRuntime.logger.error(
        '[${NotificationConfig.moduleName}] Failed to get FCM token: $e',
      );
      return null;
    }
  }
  ...
  static Future<void> unsubscribeFromTopics(List<String> topics) async {
    for (final topic in topics) {
      try {
        ...
        NotificationKitRuntime.logger.debug(
          '[${NotificationConfig.moduleName}] Unsubscribed from: $topic',
        );
      } catch (e) {
        NotificationKitRuntime.logger.error(
          '[${NotificationConfig.moduleName}] Failed to unsubscribe from $topic: $e',
        );
      }
    }
  }
  ...
  static Future<void> showNotification({
    ...
    if (imageUrl != null && imageUrl.isNotEmpty) {
      try {
        ...
        NotificationKitRuntime.logger.error(
          '[NotificationInitializer] Failed to create BigPictureStyle: $e',
        );
      }
    }
...
void _backgroundNotificationHandler(NotificationResponse response) {
  NotificationKitRuntime.logger.warning(
    '[NotificationModule] Background notification: ${response.payload}',
  );
}
