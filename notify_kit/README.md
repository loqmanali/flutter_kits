...
1. **Use classic plugin registration in AppDelegate** —
   ...
   **deaf in the foreground** (no `onMessage`, ever). This exact bug once cost a
   full day of debugging in a production app.
...
A known-good AppDelegate (trimmed from a production app):
...
- **Payload → route mapping and topic names**: backend contracts belong to
  the app (keep them in an app-side notification navigation service).
